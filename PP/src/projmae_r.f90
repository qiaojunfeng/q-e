...
!
  ...
  if (ionode) ALLOCATE ( psi(dffts%nr1x*dffts%nr2x*dffts%nr3x,nspin_mag) )
  ...
  DO ik = 1,nks
     ...
     do ibnd=1,nbnd 
        ...
        if(ionode) then
          ...
          do is = 1, nspin_mag
            psi(:, is) = dble(dist_evc_r(:,is))**2 + aimag(dist_evc_r(:,is))**2
            write(*, '(" ikpt ", i3, ", ibnd ", i3, " sum(|psi(r)|^2) = ", f18.6)') &
                  ik, ibnd, sum(psi(:,is))  ! should be 1
          end do
